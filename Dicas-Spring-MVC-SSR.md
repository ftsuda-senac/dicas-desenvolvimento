...
```html
<!-- Thymeleaf — a mensagem resolvida pelo locale aparece diretamente -->
<div class="invalid-feedback" th:if="${#fields.hasErrors('cpf')}"
     th:errors="*{cpf}"></div>
<!-- Exibe: "CPF inválido" (pt), "Invalid CPF number" (en), etc. -->
```
...
**Violações em campos específicos via `addPropertyNode()` — constraint de classe**

Em constraints de **classe** (que validam múltiplos campos ao mesmo tempo), é possível
apontar a violação para um campo específico. No Thymeleaf, isso faz o erro aparecer
no `th:errors` do campo correto — não no nível do objeto.
...
public class SenhaConfirmacaoValidator
        implements ConstraintValidator<SenhaConfirmacao, AlterarSenhaForm> {

    @Override
    public boolean isValid(AlterarSenhaForm form, ConstraintValidatorContext context) {
        if (form.getSenha() == null || form.getConfirmacaoSenha() == null) return true;
        if (form.getSenha().equals(form.getConfirmacaoSenha())) return true;

        context.disableDefaultConstraintViolation();

        // Aponta a violação para o campo confirmacaoSenha.
        // O Thymeleaf exibirá o erro no th:errors="*{confirmacaoSenha}" correto.
        context
...
```html
<!-- Thymeleaf — o erro aparece no campo confirmacaoSenha, não no objeto inteiro -->
<div class="form-group">
    <label for="confirmacaoSenha">Confirme a senha</label>
    <input type="password" id="confirmacaoSenha" th:field="*{confirmacaoSenha}"
           th:class="${#fields.hasErrors('confirmacaoSenha')} ? 'form-control is-invalid' : 'form-control'">
    <div class="invalid-feedback" th:if="${#fields.hasErrors('confirmacaoSenha')}"
         th:errors="*{confirmacaoSenha}"></div>
    <!-- Exibe: "Confirmação de senha não confere com a senha informada" -->
</div>
```
...
public String meuPerfil(@CurrentUser UsuarioInfo usuario, Model model) {
    model.addAttribute("perfil", perfilService.buscar(usuario.getId()));
...
// Uso
@PostMapping("/login")
public String login(@ModelAttribute LoginForm form, @ClientIP String ip, Model model) {
    auditService.registrarTentativaLogin(form.getEmail(), ip);
    // ...
...
// Uso
@GetMapping("/produtos")
public String listarProdutos(@TenantId String tenant, Model model) {
    model.addAttribute("produtos", produtoService.listarPorTenant(tenant));
    return "produtos/lista";
}
```
...
public String relatorios(@PaginationDefaults(size = 50) PaginationRequest paginacao, Model model) {
    model.addAttribute("dados", relatorioService.listar(paginacao.offset(), paginacao.size()));
...
> **Atenção com `{validatedValue}`:** em aplicações SSR com Thymeleaf, o valor é exibido diretamente na tela. Evite expor esse placeholder em campos sensíveis (senhas, documentos) para não vazar dados na interface.

```properties
# Usando parâmetros da anotação nas mensagens do messages.properties
Size.clienteForm.nome=Nome deve ter entre {min} e {max} caracteres
Min.produtoForm.estoque=Estoque mínimo é {value} unidade(s)
Max.produtoForm.desconto=Desconto máximo permitido é {value}%
Digits.produtoForm.preco=Preço deve ter no máximo {integer} dígitos inteiros e {fraction} decimais
Pattern.clienteForm.cep=CEP deve seguir o padrão {regexp}

# Com {validatedValue} — útil para formulários internos/backoffice
Min.pedidoForm.quantidade=Quantidade mínima é {value}, mas foi informado: {validatedValue}
```
...
public class PedidoForm {
    ...
    // {validatedValue} recebe o CEP inválido submetido pelo usuário
    @Pattern(
        regexp = "\\d{5}-\\d{3}",
...
```html
<!-- Thymeleaf — exibindo as mensagens de validação no formulário -->
<!-- th:errors resolve automaticamente a mensagem interpolada pelo Bean Validation -->
<div class="form-group">
    <label for="quantidade">Quantidade</label>
    <input type="number" id="quantidade" th:field="*{quantidade}"
           th:class="${#fields.hasErrors('quantidade')} ? 'form-control is-invalid' : 'form-control'">
    <!-- A mensagem já chega interpolada: "Quantidade mínima é 1" -->
